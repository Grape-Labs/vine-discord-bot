...
async function discordApiGet(path, accessToken, attempt = 0) {
  ...
  if (res.status === 429) {
    ...
    if (attempt >= 3) {
      throw new Error(`Discord API 429: ${JSON.stringify(body || {})}`);
    }

    const waitMs = Math.ceil(retryAfterSec * 1000) + 150;
    await sleep(waitMs);
    return discordApiGet(path, accessToken, attempt + 1);
  }
