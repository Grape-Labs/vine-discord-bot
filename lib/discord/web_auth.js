...
const GUILD_CACHE_TTL_SEC = 45; // keep short to limit stale permission windows
...
function guildsCacheKey(sessionId) {
  return `vine:web:guilds_cache:${sessionId}`;
}
...
async function getSessionGuildsCache(sessionId) {
  const kv = getKV();
  if (!kv || !sessionId) return null;

  const payload = await kv.get(guildsCacheKey(sessionId));
  if (!payload || !Array.isArray(payload.guilds)) return null;
  return payload;
}

async function setSessionGuildsCache(sessionId, guilds, ttlSec = GUILD_CACHE_TTL_SEC) {
  const kv = getKV();
  if (!kv || !sessionId || !Array.isArray(guilds)) return false;
  ...
  await kvSetWithTtl(kv, guildsCacheKey(sessionId), payload, ttlSec);
  return true;
}
