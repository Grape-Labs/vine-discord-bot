...
// - pageDelayMs: soft pacing between pages
async function fetchThreadMessages(threadId, maxMessages = 300, pageDelayMs = 450) {
  let before = null;
  let fetched = 0;
  let page = 0;
  const all = [];
  ...
  while (fetched < maxMessages) {
    ...
    // 1) Fetch one page (discordFetch should already be 429-aware if you implemented that)
    ...
    // 4) Pace the next request (reduces chance of 429)
    // Add a little jitter so multiple invocations don’t sync up
