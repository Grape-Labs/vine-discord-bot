...
async function handleAwardParticipation(interaction, res) {
  ...
  // ✅ prevent timeout warning
  defer(res, true); // ephemeral thinking…
  ...
  try {
    ...
    if (firstByUser.size === 0) {
      await discordEditOriginal(interaction, "No wallets found in this thread yet.");
      return;
    }

    const wallets = Array.from(firstByUser.values());

    // ✅ Run on-chain awards FIRST (same as your working logic)
    const results = await awardOnePoint(wallets);

    const ok = results.filter(r => r.ok);
    const bad = results.filter(r => !r.ok);

    // If everything failed, don't lock and don't post award message
    if (ok.length === 0) {
    ...
    // ✅ Only now: post lock marker + summary publicly
    ...
    const sampleTx = ok.slice(0, 5).map(r => `• \`${r.wallet}\` → ${r.url}`).join("\n");
    const failList = bad.slice(0, 5).map(r => `• \`${r.wallet}\` → ${r.error}`).join("\n");
    ...
    // ✅ finish the ephemeral response (clears “thinking” without any scary warning)
    await discordEditOriginal(
      interaction,
      `✅ Awarded **${ok.length}/${wallets.length}** participant(s). Locked for **${day}**.`
    );
  } catch (e) {
    // ✅ no public “awarded” post on error
