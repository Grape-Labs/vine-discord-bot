...
function interactionAppId(interaction) {
  return interaction?.application_id || process.env.DISCORD_APP_ID || null;
}
...
async function discordWebhookFetch(url, opts = {}, retryCount = 0) {
  ...
  if (r.status === 429) {
    ...
    if (retryCount >= 5) throw new Error(`Webhook 429 (max retries): ${text}`);
...
async function discordEditOriginal(interaction, content) {
  ...
  if (!appId) throw new Error("Missing appId (interaction.application_id or DISCORD_APP_ID)");
...
// ✅ optional: fallback progress message (separate ephemeral message)
...
async function handleAwardParticipation(interaction, res) {
  ...
  const safeProgress = async (msg) => {
    try {
      await discordEditOriginal(interaction, msg);
    } catch (e) {
      // If edit-original fails, fall back to followup so you SEE it
      try {
        await discordFollowup(interaction, msg, true);
      } catch (e2) {
        console.error("award_participation: progress failed:", e?.message, e2?.message);
      }
    }
  };
  ...
  try {
    // ✅ If your payload is missing appId/token, you’ll see it now.
    await safeProgress("⏳ Starting…");

    if (!threadId) {
      await safeProgress("❌ `/award_participation` must be used inside the participation thread.");
      return;
    }

    // ✅ Make sure bot can read private thread history
    await ensureBotInThread(threadId);

    await safeProgress("⏳ Fetching thread history…");
    const msgs = await fetchThreadMessages(threadId, 800);

    await safeProgress(`✅ Loaded ${msgs.length} messages. Scanning…`);
    ...
    if (msgs.some((m) => (m.content || "").includes(lock))) {
      await safeProgress("⛔ Participation already awarded in this thread.");
      return;
    }

    await safeProgress("⏳ Computing eligible wallets…");
    const firstByUser = buildFirstWalletByUserSince(msgs, startMsgId);
    if (firstByUser.size === 0) {
      await safeProgress("No wallets found in this thread yet.");
      return;
    }
    ...
    const daoId = (await getDaoIdForGuild(guildId)) || process.env.VINE_DAO_ID;
    if (!daoId) {
      await safeProgress("❌ This server is not configured with a Space yet. Ask an admin to run `/setspace`.");
      return;
    }

    await safeProgress(`⏳ Awarding **${wallets.length}** wallet(s) for **${day}**…`);
    ...
    if (ok.length === 0) {
      await safeProgress(`❌ Award failed (0 successes).`);
      return;
    }

    await discordPostMessage(
      threadId,
      `✅ Participation awarded for **${day}** — 1 point per participant.\n${lock}\n${awardDayMarker(day)}`,
      true
    );

    await safeProgress(`✅ Done. Awarded **${ok.length}/${wallets.length}**.`);
  } catch (e) {
    await safeProgress(`❌ Error: ${e.message}`);
  }
}
