...
// ==============================
// DROP-IN: timeout that always works (no AbortController needed)
// ==============================
function sleep(ms) {
...
// ==============================
// DROP-IN: discordFetch with timeout + 429 handling
// ==============================
async function discordFetch(path, opts = {}, retryCount = 0) {
  const token = process.env.DISCORD_BOT_TOKEN;
  if (!token) throw new Error("Missing DISCORD_BOT_TOKEN");
  ...
  const res = await fetchWithTimeout(
    ...
    12000 // give Discord a bit more time
  );
...
// ==============================
// DROP-IN: progress throttle WITHOUT setTimeout
// (setTimeout-based throttles can stall in serverless)
// ==============================
function makeNoTimerThrottle(fn, ms = 900) {
  let last = 0;
  return async (msg) => {
    const now = Date.now();
    if (now - last < ms) return; // skip updates (no scheduling)
    last = now;
...
// ==============================
// DROP-IN: fetch messages until today's start marker is found
// - newest -> older
// - stops early when marker found
// ==============================
async function fetchThreadMessagesUntilStartMarker(threadId, day, maxMessages = 800, onProgress = null) {
  ...
  const progress = onProgress ? makeNoTimerThrottle(onProgress, 900) : null;
  ...
  while (fetched < maxMessages) {
    ...
    // stop early if marker appears in this batch
...
// IMPORTANT:
// - defer(res,true) once
// - then run inline (NO setImmediate)
// - serialize edits so you don’t overlap PATCH requests
// ==============================
...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    // Fetch only enough to reach today's marker, then stop
    const msgs = await fetchThreadMessagesUntilStartMarker(
      threadId,
      day,
      800,
      async (msg) => done(msg) // progress callback
    );
