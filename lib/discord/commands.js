...
async function fetchUserVoiceChannelId(guildId, userId) {
  ...
  try {
    ...
    if (Array.isArray(list)) {
      ...
      if (hit?.channel_id) return hit.channel_id;
    }
  } catch {}
...
async function isVoiceOrStageChannel(channelId) {
  try {
    const ch = await discordFetch(`/channels/${channelId}`);
    return ch && (ch.type === 2 || ch.type === 13);
  } catch {
    return false;
  }
}
...
async function fetchUserVoiceChannelId(guildId, userId) {
  ...
  // 1) Try per-user voice state
  try {
    const vs = await discordFetch(`/guilds/${guildId}/voice-states/${userId}`);
    if (vs?.channel_id) channelId = vs.channel_id;
  } catch {
    // ignore
  }

  // 2) Fallback: scan all voice states
  if (!channelId) {
    try {
      const list = await discordFetch(`/guilds/${guildId}/voice-states`);
      if (Array.isArray(list)) {
        const hit = list.find(v => String(v?.user_id) === String(userId));
        if (hit?.channel_id) channelId = hit.channel_id;
      }
    } catch {
      // ignore
    }
  ...
  // 3) Confirm channel is Voice (2) or Stage (13)
  try {
    const ch = await discordFetch(`/channels/${channelId}`);
