...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    // ✅ Write a marker message to the thread so we can’t award again today
    // Keep it human readable + contains hidden marker token
    ...
    return reply(
      res,
      `✅ Eligible participants: **${entries.length}**\n` +
        `Award: **${amount}** point each (max 1 per user).\n` +
        `Status: **LOCKED for ${day}** (cannot run again today)\n\n` +
        `${preview}${more}`,
      false
    );
  } catch (e) {
