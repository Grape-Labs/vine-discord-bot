...
function makeThrottledProgress(fn, ms = 750) {
  ...
  return async (msg) => {
    ...
    pending = msg;
    // schedule a flush once
    if (!makeThrottledProgress._timer) {
      makeThrottledProgress._timer = setTimeout(async () => {
        makeThrottledProgress._timer = null;
        if (pending) {
          last = Date.now();
          const m = pending;
          pending = null;
...
async function fetchThreadMessages(threadId, maxMessages = 300, onProgress = null) {
  ...
  while (fetched < maxMessages) {
    ...
    if (progress) {
      await progress(`⏳ Fetching thread history… (${fetched}/${maxMessages})`);
    }
  ...
  return all; // newest->older, in fetch order (newest first)
}
...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    // 1) Voice gate (no REST spam besides voice check)
    /*
    await done("⏳ Checking voice gate…");
    const gateMsg = await requireActiveCallDeferred(interaction, "`/award_participation`");
    if (gateMsg) {
      await done(gateMsg);
      return;
    }*/

    // 2) Fetch thread history ONCE
  await done("⏳ Starting…");
  ...
  const msgs = await fetchThreadMessages(
    threadId,
    300,
    (msg) => done(msg) // progress callback
  );

  await done(`✅ Got ${msgs.length} messages. Continuing…`);

    // 3) “Started today?” check using same msgs
    const startMsgId = findTodayStartMessageId(msgs, day);
    if (!startMsgId) {
      await done(
        `❌ No participation session has been started **today** in this thread.\n` +
    ...
    await done("⏳ Check Award Lock Marker…");

    // 4) “Already awarded?” check using same msgs
    const lock = awardLockMarker();
    ...
    // 5) Build LAST wallet per user since start marker
