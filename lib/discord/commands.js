...
// Ensures we never send overlapping PATCH edits to @original
...
// Throttle progress calls (so you don't spam webhook edits)
function makeThrottledProgress(fn, ms = 900) {
  ...
  return async (msg) => {
    ...
    // otherwise store latest and schedule one flush
    pending = msg;
...
// Fetch only as far back as needed: stop once we hit today's /startparticipation marker message
async function fetchThreadMessagesUntilStartMarker(threadId, day, maxMessages = 800, onProgress = null) {
  ...
  while (fetched < maxMessages) {
    if (progress) await progress(`⏳ Fetching thread history… (${fetched}/${maxMessages})`);
    ...
    // stop early as soon as we have the marker in the fetched window
    if (msgs.some((m) => (m.content || "").includes(marker))) break;

    if (msgs.length < 100) break;
  }

  if (progress) await progress(`✅ Fetched ${all.length} messages.`);
  return all; // newest -> older
}


// ==============================
// DROP-IN REPLACEMENT:
// handleAwardParticipation
// (requires: discordEditOriginal, discordPostMessage, findTodayStartMessageId,
//            buildLastWalletByUserSince, awardLockMarker, awardDayMarker,
//            vineSpaceUrlForInteraction, getDaoIdForGuild, awardOnePoint,
//            todayUTC, startMarker, defer)
// ==============================

async function handleAwardParticipation(interaction, res) {
  // ACK immediately (ephemeral)
  defer(res, true);

  // IMPORTANT: after defer(), DO NOT call reply(res, ...) anywhere in this handler.
  const doneRaw = async (content) => {
    await discordEditOriginal(interaction, content);
  };

  // Serialize edits so Discord never sees concurrent PATCH requests
  ...
  try {
    ...
    const msgs = await fetchThreadMessagesUntilStartMarker(
      ...
      day,
      800,
      async (m) => done(m) // progress callback (queued + throttled internally)
    );
    ...
    // Lock check using the SAME fetched msgs (no extra REST calls)
    const lock = awardLockMarker();
    ...
    // Eligible wallets: LAST check-in per user since marker
