...
async function kickAwardWorkerBestEffort() {
  ...
  if (!secret) {
    console.log("award_participation: worker kick skipped (missing AWARD_WORKER_SECRET)");
    return;
  }
  ...
  try {
    ...
    if (!r.ok) console.log("award_participation: worker kick non-200", r.status);
  } catch (e) {
...
async function runAwardParticipationJob({ guildId, threadId, day }) {
  ...
  try {
    ...
    const results = await awardOnePoint(wallets, { daoId, guildId });
...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    if (!queued.ok) {
      return reply(
        res,
        `⛔ An award run is already queued or running for **${day}** in this thread.\nPlease wait for it to finish.`,
        true
      );
    }

    // Don't block the interaction response; cron is still a fallback.
    void kickAwardWorkerBestEffort();

    return reply(
      res,
      `⏳ Award run queued for **${day}**.\nI’ll post step-by-step progress in this thread shortly (usually within ~1 minute).`,
      true
    );
  } catch (e) {
    console.error("award_participation: queue error:", e.message);
    return reply(res, `❌ Failed to queue award run: ${e.message}`, true);
  }
}
