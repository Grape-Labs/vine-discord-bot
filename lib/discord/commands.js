...
async function handleAwardParticipation(interaction, res) {
  ...
  // No defer: acknowledge immediately, then post all progress in the thread.
  ...
  try {
    await post(`⏳ [1/4] Collecting check-ins for **${day}**...`);
    ...
    const daoId =
      (await withStepTimeout(getDaoIdForGuild(interaction.guild_id), "getDaoIdForGuild")) ||
      process.env.VINE_DAO_ID;
    if (!daoId) {
      await post(
    ...
    let spaceUrl = null;
    try {
      spaceUrl = await vineSpaceUrlForInteraction(interaction);
    } catch (e) {
