...
async function handleAwardParticipation(interaction, res) {
  ...
  // Clear Discord "thinking..." immediately, then continue processing.
  try {
  ...
  const respond = async (content, ephemeral = true) => {
    try {
      ...
      console.error("award_participation: followup failed:", e.message);
      try {
        await discordEditOriginal(interaction, content);
      } catch (e2) {
        console.error("award_participation: fallback edit failed:", e2.message);
      }
    }
  };
  ...
  try {
    if (!threadId) {
      return respond("❌ `/award_participation` must be used inside the participation thread.", true);
    }

    await respond(`⏳ Award run started for **${day}**. Running checks...`, true);

    // Visible marker so mods can see award execution actually started.
    try {
      await discordPostMessage(threadId, `⏳ Award run started for **${day}**. Collecting check-ins...`, true);
    } catch (e) {
      console.error("award_participation: progress marker failed:", e.message);
    }

    // Single history fetch for all remaining checks to reduce Discord API round-trips.
    const msgs = await fetchThreadMessages(threadId, THREAD_HISTORY_LIMIT);

    const awardedMarker = awardDayMarker(day);
    ...
    if (finalByUser.size === 0) {
      return respond("No wallets found in this thread yet.", false);
    }
    ...
    if (!daoId) {
      return respond(
        "❌ This server is not configured with an OG Space yet. Ask an admin to run `/setspace`.",
        true
      );
    }
    ...
    try {
      await discordPostMessage(
        threadId,
        `🛰️ Found **${wallets.length}** eligible wallet(s). Sending transactions...`,
        true
      );
    } catch (e) {
      console.error("award_participation: tx marker failed:", e.message);
    }

    // Keep per-guild signer lookup behavior for authority/payer from KV/env.
    ...
    return respond(
      `✅ Awarded **${ok.length}/${wallets.length}** participant(s) for **${day}**.`,
      true
    );
  } catch (e) {
    console.error("award_participation:error", e);
    return respond(`❌ Error: ${e.message}`, true);
  }
}
