...
const DISCORD_HTTP_TIMEOUT_MS = Math.max(
  3000,
  Math.floor(Number(process.env.DISCORD_HTTP_TIMEOUT_MS || 20000))
);
...
async function finalizeDeferredResponse(interaction, content, ephemeral = true) {
  try {
    ...
    try {
      ...
      console.error("finalizeDeferredResponse: followup failed:", e2.message);
      return false;
    }
  }
}
...
async function runAwardParticipationJob({ guildId, threadId, day }) {
  ...
  try {
    ...
    if (!daoId) {
      await post("❌ This server is not configured with an OG Space yet. Ask an admin to run `/setspace`.");
...
async function handleAwardParticipation(interaction, res) {
  ...
  if (!threadId) {
    return reply(res, "❌ `/award_participation` must be used inside the participation thread.", true);
  }
  if (!guildId) {
    return reply(res, "❌ `/award_participation` must be used in a server.", true);
  }
  ...
  let deferred = false;
  try {
    // Acknowledge quickly so Discord does not timeout, then run inline.
    await defer(res, true);
    deferred = true;

    const out = await runAwardParticipationJob({ guildId, threadId, day });

    if (out?.ok && out?.status === "completed") {
      await finalizeDeferredResponse(
        interaction,
        `✅ Award run finished for **${day}**: **${out.success}/${out.total}** success.`,
        true
      );
      return;
    }

    if (out?.ok && out?.status === "already_awarded") {
      await finalizeDeferredResponse(
        interaction,
    ...
    if (out?.ok && out?.status === "not_started") {
      await finalizeDeferredResponse(
        interaction,
        `❌ No participation session has been started **today** in this thread.`,
        true
    ...
    if (out?.ok && out?.status === "no_wallets") {
      await finalizeDeferredResponse(interaction, "ℹ️ No wallets found in this thread yet.", true);
      return;
    }

    if (out?.ok && out?.status === "missing_dao") {
      await finalizeDeferredResponse(
        interaction,
    ...
    await finalizeDeferredResponse(
      interaction,
      `❌ Award run failed for **${day}**. Check the thread for error details.`,
      true
    );
    return;
  } catch (e) {
    console.error("award_participation:error:", e.message);
    if (deferred) {
      await finalizeDeferredResponse(interaction, `❌ Failed to run award: ${e.message}`, true);
      return;
    }
    return reply(res, `❌ Failed to run award: ${e.message}`, true);
  }
}
