...
// KV key helper
function kvVoiceKey(guildId, userId) {
  return `voice:${guildId}:${userId}`;
}

async function fetchUserVoiceChannelId(guildId, userId) {
  // ✅ 0) KV first (works for voice + stage)
  try {
    const channelId = await kv.get(kvVoiceKey(guildId, userId));
    if (channelId) return String(channelId);
  } catch (e) {
    // ignore if KV not configured
  }

  // ❌ REST fallback (usually unreliable for voice state, keep as best-effort)
  try {
...
async function handleCheckin(interaction, res) {
  // ✅ ACK immediately (public message, since checkin is public)
  defer(res, false);
  ...
  try {
    ...
    if (!threadId) {
      await discordEditOriginal(interaction, "❌ `/checkin` must be used inside the participation thread.");
      return;
    }

    if (!wallet) {
      await discordEditOriginal(interaction, "❌ Missing wallet. Example: `/checkin wallet:<address>`");
      return;
    }

    if (!isValidSolanaPubkey(wallet)) {
      await discordEditOriginal(interaction, "❌ Invalid Solana wallet address.");
      return;
    }

    // If you're keeping voice gating disabled, skip requireActiveCall.
    // If you still call it, it must NOT reply(res,...) anymore since we already deferred.

    const startedGate = await requireStartedToday(interaction, null, "`/checkin`");
    if (startedGate) {
      // make requireStartedToday return a string instead of replying, OR inline the check here
      await discordEditOriginal(interaction, startedGate);
      return;
    }
    ...
    // Optional “first wallet wins” check
    try {
      const day = todayUTC();
      const msgs = await fetchThreadMessages(threadId, 300);
    ...
    await discordEditOriginal(interaction, `❌ Error: ${e.message}`);
  }
}
...
async function handleSlashCommand(interaction, res, opts = {}) {
  const name = interaction.data?.name;
  const deferred = !!opts.deferred;

  // Helper: if deferred, return content string instead of replying via res
  const out = async (content, ephemeral = true) => {
    if (deferred) return content;        // <-- return string to interactions.js
    return reply(res, content, ephemeral);
  };

  if (name === "getspace") return out(await handleGetSpace(interaction), true);
  if (name === "setspace") return out(await handleSetDao(interaction), true);
  if (name === "startparticipation") return out(await handleStartParticipation(interaction), false);
  if (name === "checkin") return out(await handleCheckin(interaction), false);
  if (name === "award_participation") return out(await handleAwardParticipation(interaction), true);

  return out(`Unknown command: ${name}`, true);
}
