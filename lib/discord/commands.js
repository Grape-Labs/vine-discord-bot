...
const DISCORD_HTTP_TIMEOUT_MS = Math.max(
  3000,
  Math.floor(Number(process.env.DISCORD_HTTP_TIMEOUT_MS || 15000))
);
...
async function handleAwardParticipation(interaction, res) {
  // Defer immediately to satisfy Discord's interaction timeout while RPC runs.
  await defer(res, true);

  try {
    await finalizeDeferredResponse(
      interaction,
      "⏳ Award run started. I’ll post progress/results in this thread.",
      true
    );
  } catch (e) {
    console.error("award_participation: failed to clear deferred spinner:", e.message);
  }
  ...
  const respond = async (content) => {
    try {
      ...
      console.error("award_participation: failed to send followup:", e.message);
    }
  };
  ...
  try {
    ...
    day = todayUTC();
    const amount = 1;
    console.log("award_participation:start", { guildId: interaction.guild_id, threadId, day });

    const gate = await requireActiveCall(interaction, null, "`/award_participation`");
    if (gate) return respond(gate);

    const startedGate = await requireStartedToday(interaction, null, "`/award_participation`");
    if (startedGate) return respond(startedGate);
    ...
    console.log("award_participation:lock_acquired", { threadId, day, lockNonce });

    // ✅ NEW: prove we're alive in the thread
    try {
      await discordPostMessage(
        threadId,
        `⏳ Award run started for **${day}**. Collecting check-ins...`,
        true
      );
    } catch (e) {
      console.error("award_participation: cannot post start message to thread:", e.message);
    }
    ...
    */
    ...
    if (finalByUser.size === 0) {
      return respond("No wallets found in this thread yet.");
    }

    const wallets = Array.from(finalByUser.values());
    console.log("award_participation:wallets", { day, count: wallets.length });

    const daoId = (await getDaoIdForGuild(interaction.guild_id)) || process.env.VINE_DAO_ID;
    if (!daoId)
      return respond("❌ This server is not configured with an OG Space yet. Ask an admin to run `/setspace`.");

    try {
      await discordPostMessage(
        threadId,
        `🛰️ Found **${wallets.length}** eligible wallet(s). Sending transactions now...`,
        true
      );
    } catch (e) {
      console.error("award_participation: cannot post pre-award message:", e.message);
    }

    const results = await awardOnePoint(wallets, { daoId, guildId: interaction.guild_id });
    console.log("award_participation:award_complete", { day, total: results.length });
    ...
    return respond(`✅ Awarded **${ok.length}/${wallets.length}** participant(s). Locked for **${day}**.`);
  } catch (e) {
