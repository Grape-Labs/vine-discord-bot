...
async function handleAwardParticipation(interaction, res) {
  ...
  const respond = async (content, ephemeral = true) => {
    try {
      await finalizeDeferredResponse(interaction, content, ephemeral);
    } catch (e) {
      console.error("award_participation: response failed:", e.message);
      try {
        await discordCreateFollowup(interaction, content, ephemeral);
      } catch (e2) {
        console.error("award_participation: followup failed:", e2.message);
      }
    }
  };
