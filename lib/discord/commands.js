...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    if (!queued.ok) {
      const kickedExisting = await kickAwardWorkerBestEffort();
      return reply(
        res,
    ...
    const kicked = await kickAwardWorkerBestEffort();
    if (!kicked) {
      try {
        await discordPostMessage(
          threadId,
          `⚠️ Award worker kick did not start immediately. The queued run will start on the next worker tick.`,
          true
        );
      } catch (_) {}
    }
