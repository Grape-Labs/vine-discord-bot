...
async function handleAwardParticipation(interaction, res) {
  const threadId = interaction.channel_id;
  const amount = 1; // ✅ fixed rule: 1 point max per user per call
  ...
  try {
    ...
    // For now: dry-run preview (next step is on-chain execution)
    return reply(
      res,
      `✅ Eligible participants: **${entries.length}**\n` +
        `Planned award: **${amount}** point each (max 1 per user).\n\n` +
        `${preview}${more}`,
      false
    );
  } catch (e) {
