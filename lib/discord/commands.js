...
async function handleAwardParticipation(interaction, res) {
  ...
  // Helper: always try to tell the user *something*.
  // Order: edit original -> ephemeral followup -> thread message fallback
  const notify = async (content, opts = {}) => {
    const { ephemeral = true, alsoThread = false } = opts;

    // 1) Try edit original (best UX)
    try {
      await discordEditOriginal(interaction, content);
      if (alsoThread && threadId) {
        try { await discordPostMessage(threadId, content, true); } catch (_) {}
      }
      return;
    } catch (e) {
      console.error("award_participation: notify edit failed:", e.message);
    }
    ...
    try {
      ...
      console.error("award_participation: notify followup failed:", e.message);
    }
    ...
    if (threadId) {
      try {
        ...
        console.error("award_participation: notify thread fallback failed:", e.message);
      }
    }
  };

  // Immediately clear the spinner with a status message.
  await notify("⏳ Award run started. I’ll post progress/results in this thread.", { ephemeral: true });
  ...
  try {
    ...
    // Voice gate + started-today gates
    ...
    await notify(`⏳ Award run started for **${day}**. Collecting check-ins...`, { ephemeral: true, alsoThread: true });
    ...
    await notify(`🛰️ Found **${wallets.length}** eligible wallet(s). Sending transactions now...`, { ephemeral: true, alsoThread: true });
    ...
    return notify(`❌ Error: ${e.message}`, { ephemeral: true, alsoThread: true });
  } finally {
