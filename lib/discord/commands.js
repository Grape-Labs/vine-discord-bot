...
const REQUIRE_VOICE = process.env.VINE_REQUIRE_VOICE === "1";
...
async function requireActiveCall(interaction, res, labelForUser) {
  const guildId = interaction.guild_id;
  const userId = interaction.member?.user?.id || interaction.user?.id;
  if (!REQUIRE_VOICE) return null; // ✅ disabled
...
async function handleCheckin(interaction, res) {
  ...
  try {
    ...
    const startedGate = await requireStartedToday(interaction, "`/checkin`");
    if (startedGate) {
...
async function requireStartedToday(interaction, labelForUser) {
  const threadId = interaction.channel_id;
  if (!threadId) {
    return `❌ ${labelForUser} must be used inside the participation thread.`;
  }
...
async function handleAwardParticipation(interaction, res) {
  ...
  const startedGate = await requireStartedToday(interaction, "`/award_participation`");
    if (startedGate) {
        await discordEditOriginal(interaction, startedGate);
        return;
    }
