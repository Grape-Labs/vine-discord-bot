...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    // Lock first (prevents double-click re-award)
    ...
    // Post a public summary (optionally include a few tx links)
    ...
    await discordPostMessage(
      ...
      summary +
        (sampleTx ? `\n\nSample tx:\n${sampleTx}` : "") +
        (failList ? `\n\nLeaving out ${Math.max(0, bad.length - 5)} more failures.\nFirst failures:\n${failList}` : "")
    );

    return reply(res, `Started awarding **${wallets.length}** participant(s). See thread for results.`, true);
  } catch (e) {
