...
function parseCheckinOverride(content) {
  // Matches exactly: VINE_CHECKIN_OVERRIDE:YYYY-MM-DD:<userId>:<wallet>
  const m = String(content || "").match(
    /VINE_CHECKIN_OVERRIDE:(\d{4}-\d{2}-\d{2}):(\d+):([1-9A-HJ-NP-Za-km-z]{32,44})/
  );
  if (!m) return null;
...
async function handleCheckin(interaction, res) {
  ...
  const fix = interaction.data?.options?.find((o) => o.name === "fix")?.value === true;
  ...
  try {
    ...
    if (finalByUser.has(userId) && fix) {
    // record an override marker (bot message) so award uses the corrected wallet
