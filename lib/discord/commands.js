...
async function handleGetSpace(interaction, res) {
  ...
  const daoId = getDaoIdForGuild(guildId) || process.env.VINE_DAO_ID;
...
async function handleSetDao(interaction, res) {
  ...
  // TODO: add role/permission check here (admin/mod)
  // For now, at least block random users later once you add role gating.

  setDaoIdForGuild(guildId, daoId);
...
function vineSpaceUrlForInteraction(interaction) {
  const guildId = interaction?.guild_id;
  const daoId = getDaoIdForGuild(guildId) || process.env.VINE_DAO_ID; // fallback for now
...
async function handleAwardParticipation(interaction, res) {
  const spaceUrl = vineSpaceUrlForInteraction(interaction);
  const threadId = interaction.channel_id;
  const day = todayUTC();
  const amount = 1;
  ...
  try {
    ...
    const daoId = getDaoIdForGuild(interaction.guild_id) || process.env.VINE_DAO_ID;
