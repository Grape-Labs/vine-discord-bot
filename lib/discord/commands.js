...
function replyJson(res, body) {
  res.status(200).setHeader("Content-Type", "application/json").end(JSON.stringify(body));
}

// Discord interaction response types:
// 4 = CHANNEL_MESSAGE_WITH_SOURCE
function replyText(res, content, flags) {
  return replyJson(res, { type: 4, data: { content, flags } });
}
...
function helpText({ spaceUrl, daoId, threadId, voiceGateOn, voiceGateChannelId }) {
  ...
  lines.push(`• \`/points\` — show your Vine points balance`);
  lines.push(`• \`/leaderboard\` — top wallets by points`);
  lines.push(`• \`/getspace\` — show configured Vine space`);
  lines.push(`• \`/setauthority\` — set this server’s authority signer (admin)`);
...
function awardInProgressMarker(day, nonce) {
  return `VINE_AWARD_IN_PROGRESS:${day}:${nonce}`;
}
...
async function hasAwardInProgressForDay(threadId, day) {
  ...
  return msgs.some((m) => parseAwardInProgress(m.content)?.day === day);
}
...
async function handleLeaderboard(interaction, res) {
  ...
  try {
    ...
    return reply(
      res,
      `📊 **Vine Leaderboard** (Season **${lb.season}**)\n` +
        `• Space: ${url}\n\n` +
        lines.join("\n"),
      ephemeral
    );
...
async function handleSetAuthority(interaction, res) {
  ...
  const authoritySecret = getOption(interaction, "authority_secret");
  const payerSecret = getOption(interaction, "payer_secret") || null;
  const rpcUrl = getOption(interaction, "rpc_url") || null;

  if (!authoritySecret) {
    return reply(
      res,
      "❌ Missing authority secret. Example: `/setauthority authority_secret:<BASE58_SECRET>`",
      true
    );
  }

  try {
    const meta = await setSignerForGuild(guildId, {
      authoritySecret,
      payerSecret,
      rpcUrl,
      updatedBy: userId,
    ...
    return reply(
      res,
      `✅ Authority signer saved for this server.\n` +
        `• Authority: \`${meta.authorityPublicKey}\`\n` +
        `• Payer: \`${meta.payerPublicKey}\`\n` +
        `• RPC: ${meta.rpcUrl || "_default (`SOLANA_RPC_URL`)_"}\n` +
        `• Updated by: <@${userId}>`,
      true
    );
  } catch (e) {
    return reply(res, `❌ Failed to save authority: ${e.message}`, true);
  }
}
    ...
    if (!meta) {
      return reply(
        ...
        "ℹ️ No authority signer is configured for this server yet.\n" +
          "Run `/setauthority authority_secret:<BASE58_SECRET>` first.",
        true
      );
    }
