...
async function discordEditOriginal(interaction, content) {
  ...
  const r = await discordWebhookFetch(url, {
    method: "PATCH",
...
async function discordWebhookFetch(url, opts = {}, retryCount = 0) {
  ...
  if (r.status === 429) {
    ...
    if (retryCount >= 5) {
      throw new Error(`Webhook 429 (max retries): ${text}`);
    }

    const waitMs = Math.ceil(retryAfter * 1000) + 100;
    await new Promise((res) => setTimeout(res, waitMs));
    return discordWebhookFetch(url, opts, retryCount + 1);
  }
...
async function discordFetch(path, opts = {}, retryCount = 0) {
  ...
  if (res.status === 429) {
    ...
    if (retryCount >= 5) throw new Error(`Discord API 429 (max retries): ${text}`);
...
function makeThrottledProgress(fn, ms = 750) {
  let last = 0;
  let timer = null;
  let pending = null;
  ...
  return async (msg) => {
    ...
    if (now - last >= ms) {
      last = now;
      try { await fn(msg); } catch (e) {}
      return;
    }

    // Otherwise remember latest message and schedule one flush
    pending = msg;
...
// Fetch messages in this thread (paginated)
async function fetchThreadMessages(threadId, maxMessages = 300, onProgress = null) {
  let before = null;
  let fetched = 0;
  const all = [];

  const progress = onProgress ? makeThrottledProgress(onProgress, 750) : null;

  if (progress) progress(`⏳ Fetching thread history… (0/${maxMessages})`);

  while (fetched < maxMessages) {
    const qs = new URLSearchParams({ limit: "100" });
    if (before) qs.set("before", before);

    if (progress) progress(`⏳ Fetching thread history… (${fetched}/${maxMessages})`);
  ...
  if (progress) progress(`✅ Fetched ${all.length} messages.`);
  return all;
}
...
// Build LAST wallet per user, but ONLY from checkins posted AFTER startMessageId
function buildLastWalletByUserSince(messages, startMessageId) {
  const startId = msgIdBigInt(startMessageId);
  if (!startId) return new Map();

  // messages are newest -> oldest (Discord returns newest first)
  // If we iterate in that order, the FIRST time we see a user's checkin is their latest.
  const lastByUser = new Map();

  for (const m of messages) {
    const mid = msgIdBigInt(m.id);
    if (!mid || mid <= startId) continue; // ignore before/at start
    ...
    // since we are going newest->oldest, first seen is the most recent checkin
    if (lastByUser.has(uid)) continue;

    const wallet = extractCheckinWallet(m.content);
    if (wallet) lastByUser.set(uid, wallet);
  }
...
// Optional: if they already checked in today, tell them we updated it (but still allow it)
try {
  const day = todayUTC();
  const msgs = await fetchThreadMessages(threadId, 200);

  const startMsgId = findTodayStartMessageId(msgs, day);
  if (startMsgId) {
    const lastByUser = buildLastWalletByUserSince(msgs, startMsgId);
    if (lastByUser.has(userId)) {
      const prev = lastByUser.get(userId);
      // NOTE: lastByUser includes the latest checkin currently in the thread history
      // This check runs BEFORE we post the new checkin, so prev is the “previous latest”
      // Not required, but nice UX.
      return reply(
        res,
        `🔁 <@${userId}> updated check-in wallet:\n` +
          `• Previous: \`${prev}\`\n` +
          `• New: \`${wallet}\``,
        false
      );
...
/*
  try {
    ...
    const startMsgId = findTodayStartMessageId(msgs, day);
    if (!startMsgId) {
      // this should be rare because requireStartedToday already checked,
      // but keep it safe
      return reply(
        res,
...
async function requireActiveCallDeferred(interaction, labelForUser) {
  const guildId = interaction.guild_id;
  const userId = interaction.member?.user?.id || interaction.user?.id;

  if (!guildId || !userId) {
    return `❌ ${labelForUser} is only available inside the server thread while you are in the active call.`;
  }
  ...
  if (!channelId) {
    return `❌ You must be in the active voice/stage call to use ${labelForUser}.` +
      (ACTIVE_VOICE_CHANNEL_ID ? `\n(Required channel: \`${ACTIVE_VOICE_CHANNEL_ID}\`)` : "");
  }
...
async function requireStartedTodayDeferred(interaction, labelForUser) {
  const threadId = interaction.channel_id;
  if (!threadId) return `❌ ${labelForUser} must be used inside the participation thread.`;

  const started = await hasStartMarkerToday(threadId);
  if (!started) {
    return `❌ No participation session has been started **today** in this thread.\n` +
      `Ask a moderator to run \`/startparticipation\` first.`;
  }
...
// ==============================
// DROP-IN HELPERS (add once)
// ==============================

function makeEditQueue(editFn) {
  let chain = Promise.resolve();
  return (content) => {
    chain = chain.then(() => editFn(content)).catch(() => {});
    return chain;
  };
}

function makeThrottledProgress(fn, ms = 900) {
  let last = 0;
  let timer = null;
  let pending = null;
...
// ==============================
// DROP-IN: fetchThreadMessagesUntilStartMarker
// - Fetches newest -> older
// - Stops early when today's start marker is found
// - Calls onProgress (throttled) so your "done(...)" updates show
// ==============================

async function fetchThreadMessagesUntilStartMarker(threadId, day, maxMessages = 800, onProgress = null) {
  const marker = startMarker(day);
  ...
  const progress = onProgress ? makeThrottledProgress(onProgress, 900) : null;

  if (progress) await progress(`⏳ Fetching thread history… (0/${maxMessages})`);
  ...
  while (fetched < maxMessages) {
    ...
    if (progress) await progress(`⏳ Fetching page ${page}… (${fetched}/${maxMessages})`);

    // NOTE: discordFetch already has timeout+429 retry in your code
    const msgs = await discordFetch(`/channels/${threadId}/messages?${qs.toString()}`);
    ...
    // stop early if marker appears in this batch (or anywhere fetched so far)
    if (msgs.some((m) => (m.content || "").includes(marker))) {
      if (progress) await progress(`✅ Found today's marker. Fetched ${all.length} messages.`);
      return all;
    }

    // if less than 100, end
    if (msgs.length < 100) break;

    // tiny yield so webhook edits can flush smoothly in some runtimes
    await sleep(25);
  }

  if (progress) await progress(`✅ Fetched ${all.length} messages.`);
  return all;
}


// ==============================
// DROP-IN: handleAwardParticipation
// IMPORTANT: Detaches the heavy work via setImmediate()
// so serverless runtimes don't freeze you right after defer().
// ==============================

async function handleAwardParticipation(interaction, res) {
  // 1) Respond to Discord immediately (deferred ephemeral)
  // This MUST be the only HTTP response.
  defer(res, true);

  // 2) Detach the rest of the work from the HTTP request lifecycle
  setImmediate(async () => {
    const doneRaw = async (content) => {
      await discordEditOriginal(interaction, content);
    };

    // serialize edits so we never overlap PATCH calls
    const done = makeEditQueue(async (content) => {
      try {
        await doneRaw(content);
      } catch (e) {
        console.error("award_participation: edit original failed:", e?.stack || e?.message || e);
      }
    });
    ...
    try {
      if (!threadId) {
        await done("❌ `/award_participation` must be used inside the participation thread.");
        return;
      }

      await done("⏳ Starting…");

      // Fetch only enough messages to include today's marker
      const msgs = await fetchThreadMessagesUntilStartMarker(
        threadId,
        day,
        800,
        async (msg) => done(msg) // progress callback (queued + throttled)
      );

      await done(`✅ Loaded ${msgs.length} messages. Scanning…`);

      // Find today's start marker message id
      const startMsgId = findTodayStartMessageId(msgs, day);
      if (!startMsgId) {
        await done(
          `❌ No participation session has been started **today** in this thread.\n` +
      ...
      // Lock check (NO extra REST call)
      const lock = awardLockMarker();
      if (msgs.some((m) => (m.content || "").includes(lock))) {
        await done(`⛔ Participation was already awarded in this thread.\n(One award per thread only.)`);
        return;
      }

      // LAST wallet per user since start marker
      await done("⏳ Computing eligible wallets…");
      const lastByUser = buildLastWalletByUserSince(msgs, startMsgId);

      if (lastByUser.size === 0) {
        await done("No wallets found in this thread yet.");
        return;
      }

      const wallets = Array.from(lastByUser.values());

      // DAO
      const daoId = (await getDaoIdForGuild(guildId)) || process.env.VINE_DAO_ID;
      if (!daoId) {
        await done("❌ This server is not configured with a Space yet. Ask an admin to run `/setspace`.");
        return;
      }

      await done(`⏳ Awarding **${wallets.length}** wallet(s) for **${day}**…`);
      ...
      await done("⏳ Posting results to thread…");
      ...
      await done(`✅ Awarded **${ok.length}/${wallets.length}** participant(s). Locked for **${day}**.`);
    } catch (e) {
