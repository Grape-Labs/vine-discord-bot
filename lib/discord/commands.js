...
async function handleAwardParticipation(interaction, res) {
  ...
  // Clear spinner once, then avoid repeated @original edits.
  try {
  ...
  try {
    ...
    await postThread(`⏳ Award run started for **${day}**. Running checks...`);
    ...
    await postThread(`⏳ Award run started for **${day}**. Collecting check-ins...`);
    ...
    await postThread(`🛰️ Found **${wallets.length}** eligible wallet(s). Sending transactions now...`);
