...
const done = async (content) => {
    try {
      ...
      console.error(
        "award_participation: edit original failed:",
        e?.stack || e?.message || e
      );
    }
  };
  await done("⏳ Starting award… (fetching check-ins)");
  ...
  try {
    // NOTE: after defer(), DO NOT call reply(res, ...) anywhere.
    // Everything must go through done() and/or discordPostMessage().

    // ✅ gating checks should return strings instead of replying
    const gateMsg = await requireActiveCallDeferred(interaction, "`/award_participation`");
    if (gateMsg) return done(gateMsg);

    const startedMsg = await requireStartedTodayDeferred(interaction, "`/award_participation`");
    if (startedMsg) return done(startedMsg);

    const already = await hasAwardMarker(threadId);
    if (already) {
      return done(`⛔ Participation was already awarded in this thread.\n(One award per thread only.)`);
    }
    ...
    // Use LAST wallet per user since start (if you switched to last-wins)
    const lastByUser = buildLastWalletByUserSince(msgs, startMsgId);
    if (lastByUser.size === 0) {
      return done("No wallets found in this thread yet.");
    }
    ...
    if (!daoId) {
      return done("❌ This server is not configured with an OG Space yet. Ask an admin to run `/setspace`.");
    }

    // Tell the invoker we’re working
    ...
    // Post lock marker + summary into thread
    ...
    // Final response to invoker (ephemeral)
    return done(`✅ Awarded **${ok.length}/${wallets.length}** participant(s). Locked for **${day}**.`);
  } catch (e) {
