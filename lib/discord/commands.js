...
async function discordFetch(path, opts = {}, retryCount = 0) {
  ...
  if (res.status === 429) {
    ...
    try {
      const data = text ? JSON.parse(text) : null;
      // Discord returns seconds (can be fractional)
      retryAfter = Number(data?.retry_after ?? 1);
    } catch {}

    // cap retries so we don’t hang forever
    if (retryCount >= 5) {
      throw new Error(`Discord API 429 (max retries reached): ${text}`);
    }

    // small padding so we don’t hit the bucket edge
    const waitMs = Math.ceil(retryAfter * 1000) + 50;
...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    await done("⏳ Fetching thread check-ins…");
    const msgs = await fetchThreadMessages(threadId, 100);

    await done("⏳ Find Today Start Message Id…");
