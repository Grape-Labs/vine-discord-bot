...
async function handleAwardParticipation(interaction, res) {
  const threadId = interaction.channel_id;
  const amount = 1; // fixed rule
  const day = todayUTC();
  const marker = awardMarker(day);

  try {
    // ✅ block if already awarded today (no DB needed)
    ...
    const wallets = entries.map(([, w]) => w);

    // ✅ Write marker FIRST to prevent double-award from two people clicking at once
    // If awards fail, we’ll post a follow-up with failures.
    await discordPostMessage(
      threadId,
      `✅ Participation awarding started for **${day}** — **${amount}** point per participant.\n${marker}`
    );
    ...
    const failPreview = fail
      .slice(0, 8)
      .map((r) => `❌ \`${r.wallet}\` → ${r.error}`)
      .join("\n");

    // Post summary to the thread (public)
    await discordPostMessage(
      threadId,
      `🏁 Participation award complete for **${day}**\n` +
        `Eligible: **${wallets.length}** | Awarded: **${ok.length}** ✅ | Failed: **${fail.length}** ❌\n` +
        (okPreview ? `\n**Success (first 8):**\n${okPreview}\n` : "") +
        (failPreview ? `\n**Failures (first 8):**\n${failPreview}\n` : "")
    );

    // Also respond to the slash command quickly
    return reply(
      res,
      `✅ Award processed for **${day}**.\nAwarded: **${ok.length}** ✅ | Failed: **${fail.length}** ❌\n` +
        `Thread is **LOCKED for ${day}** (marker posted).`,
      false
    );
  } catch (e) {
