...
async function discordFetch(path, opts = {}, retryCount = 0) {
  ...
  if (res.status === 429) {
    ...
    await new Promise((r) => setTimeout(r, Math.ceil(retryAfter * 1000) + 150));
...
// IMPORTANT: join the thread before reading history
async function ensureThreadMember(threadId) {
  try {
    // Discord returns 204 on success. Sometimes you’ll get 409 if already a member.
    await discordFetch(`/channels/${threadId}/thread-members/@me`, { method: "PUT" });
  } catch (e) {
    // If already joined, Discord may throw 409 which discordFetch would treat as error.
    // So we special-case that by retrying with raw fetch to inspect status.
    const token = process.env.DISCORD_BOT_TOKEN;
    const res = await fetchWithTimeout(`${DISCORD_API}/channels/${threadId}/thread-members/@me`, {
      method: "PUT",
      headers: { Authorization: `Bot ${token}` },
    });

    if (res.status === 409 || res.status === 204) return; // already in / joined
    const txt = await res.text().catch(() => "");
    throw new Error(`ensureThreadMember failed ${res.status}: ${txt}`);
  }
}
...
async function discordEditOriginal(interaction, content, retryCount = 0) {
  const appId = interactionAppId(interaction);
  const token = interaction.token;
  if (!appId) throw new Error("Missing DISCORD_APP_ID");
  if (!token) throw new Error("Missing interaction.token");
  ...
  if (res.status === 429) {
    ...
    if (retryCount >= 5) throw new Error(`editOriginal 429 (max retries): ${text}`);
    await new Promise((r) => setTimeout(r, Math.ceil(retryAfter * 1000) + 150));
    return discordEditOriginal(interaction, content, retryCount + 1);
  }
...
async function fetchThreadMessages(threadId, maxMessages = 300, onProgress = null) {
  await ensureThreadMember(threadId); // ✅ critical for threads

  let before = null;
  let fetched = 0;
  const all = [];

  const maybeProgress = async (msg) => {
    if (!onProgress) return;
    try { await onProgress(msg); } catch {}
  };

  await maybeProgress(`⏳ Fetching thread history… (0/${maxMessages})`);
  ...
  while (fetched < maxMessages) {
    ...
    await maybeProgress(`⏳ Fetching thread history… (${fetched}/${maxMessages})`);

    if (msgs.length < 100) break;
  }

  await maybeProgress(`✅ Fetched ${all.length} messages.`);
  return all;
}
...
async function handleAwardParticipation(interaction, res) {
  ...
  try {
    ...
    // IMPORTANT: join thread + show progress during fetch
    const msgs = await fetchThreadMessages(threadId, 800, async (msg) => {
      // don’t spam edits too fast
      await done(msg);
    });
