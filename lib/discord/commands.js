...
function defer(res, ephemeral = true) {
  const payload = { type: 5, data: {} }; // DEFERRED_CHANNEL_MESSAGE_WITH_SOURCE
...
async function handleAwardParticipation(interaction, res) {
  ...
  // ✅ ACK immediately so Discord never shows “did not respond”
  defer(res, true); // ephemeral “thinking…”

  // Everything below runs after we already responded
  (async () => {
    try {
      ...
      if (firstByUser.size === 0) {
        await discordPostMessage(threadId, `No wallets found in this thread yet.`);
        return;
      }
      ...
      // ✅ Only lock once we have successes
      await discordPostMessage(
        threadId,
      ...
      await discordPostMessage(threadId, `❌ Error during awarding: ${e.message}`);
    }
  })();
}
